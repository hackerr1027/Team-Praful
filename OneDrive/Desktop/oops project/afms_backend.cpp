...
public:
    ...
    Vehicle(string id_, string name_, string cargo_, Route route_)
        : id(id_), name(name_), cargo(cargo_), route(route_), 
          current_waypoint_idx(0), total_distance(0.0), speed(0.0), status("Active") {
    ...
    virtual void move() {
        if (route.waypoints.empty()) return;
        ...
        if (distance > 0) {
            ...
            total_distance += calculateDistance(lat - move_lat, lon - move_lon, lat, lon) * 111.0; // deg to km
            
            // Set realistic speed with some variation
            double base_speed = getBaseSpeed();
            speed = base_speed + (rand() % 10 - 5); // +/- 5 km/h variation
            speed = max(0.0, min(speed, route.speed_limit + 10)); // Cap at speed limit + 10
        }
    ...
    string getCurrentDestination() const {
        if (route.waypoints.empty()) return "Unknown";
        int next_idx = (current_waypoint_idx + 1) % route.waypoints.size();
        return route.waypoints[next_idx].name;
    }
    
    int getETA() const {
        if (route.waypoints.empty() || speed == 0) return 0;
        int next_idx = (current_waypoint_idx + 1) % route.waypoints.size();
        double dist = calculateDistance(lat, lon, 
            route.waypoints[next_idx].lat, route.waypoints[next_idx].lon) * 111.0;
...
bool postTelemetry(const json& js, const string& url) {
    ...
    fout << js.dump();
    fout.close();
    string cmd = "curl.exe -X POST \"" + url + "\" -H \"Content-Type: application/json\" -d @" + tmpFile + " >nul 2>&1";
